...
CANDIDATE_WIN_COLOR: tuple[float, float, float] = (0.02, 0.38, 0.95)
BASELINE_WIN_COLOR: tuple[float, float, float] = (0.95, 0.08, 0.04)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--winner-alpha-full-rmse-delta",
        default=float(config_default(config, "winner_alpha_full_rmse_delta", WINNER_ALPHA_FULL_RMSE_DELTA)),
        type=float,
        help="8-bit local RMSE gap that reaches --winner-alpha-max.",
    )
...
def build_local_error_maps(
    ...
    return candidate_local_error, baseline_local_error


def build_local_winner_masks_from_errors(
    candidate_local_error: np.ndarray,
    baseline_local_error: np.ndarray,
    confidence_threshold: float,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    validate_same_shape("baseline_local_error", candidate_local_error, baseline_local_error)
...
def build_local_rmse_delta_map(candidate_local_error: np.ndarray, baseline_local_error: np.ndarray) -> np.ndarray:
    validate_same_shape("baseline_local_error", candidate_local_error, baseline_local_error)
    candidate_local_rmse = np.sqrt(np.maximum(candidate_local_error, 0.0))
    baseline_local_rmse = np.sqrt(np.maximum(baseline_local_error, 0.0))
    return np.abs(baseline_local_rmse - candidate_local_rmse)
...
def build_local_winner_masks(
    ...
    candidate_local_error, baseline_local_error = build_local_error_maps(
        target=target,
        candidate_prediction=candidate_prediction,
        baseline_prediction=baseline_prediction,
        kernel_size=kernel_size,
    )
    return build_local_winner_masks_from_errors(
        candidate_local_error=candidate_local_error,
        baseline_local_error=baseline_local_error,
...
def blend_color_on_white(color: tuple[float, float, float], alpha: float) -> tuple[float, float, float]:
    color_array = np.array(color, dtype=np.float32)
    white = np.ones(3, dtype=np.float32)
    swatch = (1.0 - alpha) * white + alpha * color_array
    return float(swatch[0]), float(swatch[1]), float(swatch[2])
...
def build_winner_alpha_map(
    ...
    if winner_mask.dtype != bool:
        raise TypeError(f"Winner mask must be bool: dtype={winner_mask.dtype}")
    if not 0.0 <= winner_alpha_max <= 1.0:
        raise ValueError(f"winner_alpha_max must be between 0 and 1: value={winner_alpha_max}")
    if winner_alpha_full_rmse_delta <= 0.0:
        raise ValueError(f"winner_alpha_full_rmse_delta must be positive: value={winner_alpha_full_rmse_delta}")

    validate_same_shape("winner_mask", local_rmse_delta, winner_mask)
    full_delta = winner_alpha_full_rmse_delta / 255.0
    alpha_strength = np.clip(local_rmse_delta / full_delta, 0.0, 1.0)
    alpha_map = np.zeros_like(local_rmse_delta, dtype=np.float32)
    alpha_map[winner_mask] = alpha_strength[winner_mask] * winner_alpha_max
    return alpha_map


def build_constant_alpha_map(mask: np.ndarray, alpha: float) -> np.ndarray:
    if mask.dtype != bool:
        raise TypeError(f"Mask must be bool: dtype={mask.dtype}")
    if not 0.0 <= alpha <= 1.0:
        raise ValueError(f"alpha must be between 0 and 1: value={alpha}")

    return np.full(mask.shape, alpha, dtype=np.float32)


def blend_color_by_alpha(
    image: np.ndarray,
    mask: np.ndarray,
    ...
    if image.shape[:2] != mask.shape:
        raise ValueError(f"Mask shape mismatch: image_shape={image.shape} mask_shape={mask.shape}")
    validate_same_shape("alpha_map", mask, alpha_map)
    ...
    color_array = np.array(color, dtype=np.float32)
    alpha_values = alpha_map[mask].astype(np.float32)[:, np.newaxis]
    output[mask] = (1.0 - alpha_values) * output[mask] + alpha_values * color_array
    return output
...
def draw_overlay_legend(
    ...
    legend_items = (
        (blend_color_on_white(CANDIDATE_WIN_COLOR, winner_alpha_max), f"blue: {candidate_name} better"),
        (blend_color_on_white(BASELINE_WIN_COLOR, winner_alpha_max), f"red: {baseline_name} better"),
        (blend_color_on_white(NEUTRAL_COLOR, neutral_alpha), "gray: small difference"),
    )
    ...
    for index, text in enumerate(text_lines):
        cv2.putText(canvas, text, (x0, 250 + index * 30), cv2.FONT_HERSHEY_SIMPLEX, 0.50, (0.05, 0.05, 0.05), 1, cv2.LINE_AA)
...
def build_local_winner_overlay(
    ...
    winner_alpha_map = build_winner_alpha_map(
        ...
        winner_mask=candidate_better | baseline_better,
        winner_alpha_max=winner_alpha_max,
        winner_alpha_full_rmse_delta=winner_alpha_full_rmse_delta,
    )
    neutral_alpha_map = build_constant_alpha_map(mask=neutral, alpha=neutral_alpha)
    overlay = blend_color_by_alpha(image=overlay, mask=candidate_better, color=CANDIDATE_WIN_COLOR, alpha_map=winner_alpha_map)
    overlay = blend_color_by_alpha(image=overlay, mask=baseline_better, color=BASELINE_WIN_COLOR, alpha_map=winner_alpha_map)
    overlay = blend_color_by_alpha(image=overlay, mask=neutral, color=NEUTRAL_COLOR, alpha_map=neutral_alpha_map)
...
def save_sample_overlay_to_path(
    ...
    candidate_better, baseline_better, neutral = build_local_winner_masks_from_errors(
        candidate_local_error=candidate_local_error,
        baseline_local_error=baseline_local_error,
        confidence_threshold=confidence_threshold,
    )
    local_rmse_delta = build_local_rmse_delta_map(candidate_local_error=candidate_local_error, baseline_local_error=baseline_local_error)
