...
class Model(nn.Module):
    ...
    def inference(self, img0, img1, embt, scale_factor=1.0):
        ...
        # imgt_, _ = pad_to_multiple(imgt_, 8, mode="replicate")
        ...
        # ---- crop predictions back to original H,W for losses/outputs ----
        imgt_pred_c = crop_like(imgt_pred, H, W)
        up_flow0_1_c = crop_like(up_flow0_1, H, W)
        up_flow1_1_c = crop_like(up_flow1_1, H, W)
        up_mask_1_c = crop_like(up_mask_1, H, W)

        return imgt_pred_c, up_flow0_1_c, up_flow1_1_c, up_mask_1_c


    def forward(self, img0, img1, embt, imgt, flow=None):
        H, W = img0.shape[-2:]  # original

        # mean on original (scalar), then pad
        mean_ = torch.cat([img0, img1], 2).mean(1, keepdim=True).mean(2, keepdim=True).mean(3, keepdim=True)
        img0 = img0 - mean_
        img1 = img1 - mean_
        imgt_ = imgt - mean_
        ...
        # ---- original network forward (unchanged) ----
        f0_1, f0_2, f0_3, f0_4 = self.encoder(img0)
        f1_1, f1_2, f1_3, f1_4 = self.encoder(img1)
        ...
        loss_rec = self.l1_loss(imgt_pred_c - imgt) + self.tr_loss(imgt_pred_c, imgt)
        loss_geo = 0.01 * (self.gc_loss(ft_1_, ft_1) + self.gc_loss(ft_2_, ft_2) + self.gc_loss(ft_3_, ft_3))

        if flow is not None:
            flow_c = crop_like(flow, H, W)
            robust_weight0 = get_robust_weight(up_flow0_1_c, flow_c[:, 0:2], beta=0.3)
            robust_weight1 = get_robust_weight(up_flow1_1_c, flow_c[:, 2:4], beta=0.3)

            # keep your original multi-scale formulation (works fine because tensors are internally consistent)
            up_flow0_2_c = crop_like(2.0 * resize(up_flow0_2, 2.0), H, W)
            up_flow1_2_c = crop_like(2.0 * resize(up_flow1_2, 2.0), H, W)
            up_flow0_3_c = crop_like(4.0 * resize(up_flow0_3, 4.0), H, W)
            up_flow1_3_c = crop_like(4.0 * resize(up_flow1_3, 4.0), H, W)
            up_flow0_4_c = crop_like(8.0 * resize(up_flow0_4, 8.0), H, W)
            up_flow1_4_c = crop_like(8.0 * resize(up_flow1_4, 8.0), H, W)
            loss_dis = 0.01 * (
                self.rb_loss(up_flow0_2_c - flow_c[:, 0:2], weight=robust_weight0) +
                self.rb_loss(up_flow1_2_c - flow_c[:, 2:4], weight=robust_weight1)
            )
            loss_dis += 0.01 * (
                self.rb_loss(up_flow0_3_c - flow_c[:, 0:2], weight=robust_weight0) +
                self.rb_loss(up_flow1_3_c - flow_c[:, 2:4], weight=robust_weight1)
            )
            loss_dis += 0.01 * (
                self.rb_loss(up_flow0_4_c - flow_c[:, 0:2], weight=robust_weight0) +
                self.rb_loss(up_flow1_4_c - flow_c[:, 2:4], weight=robust_weight1)
            )
        else:
            loss_dis = 0.00 * loss_geo

        return imgt_pred_c, loss_rec, loss_geo, loss_dis, up_flow0_1_c, up_flow1_1_c, up_mask_1_c
